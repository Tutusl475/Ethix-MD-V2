    ...
    try {
      ...
      const buttons = videoQualities.map((video, index) => ({
        ...
        "buttonParamsJson": JSON.stringify({
          display_text: `🧚‍♂️ Download 📥 ${video.resolution}`,
      ...
      const sections = videoQualities.map((video) => ({
        ...
        rows: [{
          title: `🧚‍♂️ Download 📥 ${video.resolution}`,
      ...
      await m.reply('🧚‍♂️ 𝔼ℝℝ𝕆ℝ, 𝕋ℝ𝕐 𝔸𝔾𝔸𝕀ℕ.');
      await m.React("❌");
    }
  } else if (selectedId) { 
    if (selectedId.startsWith('fbmedia_')) {
      ...
      if (selectedMedia) {
        try {
          ...
          console.error("Error processing your request:", error);
          await m.reply('🧚‍♂️ Error, please try again');
          await m.React("❌");
        }
      }
    }
